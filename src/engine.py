...
from src.models_deterministic import DemandPredictor
...
class RevenueEngine:
    ...
    def run(self, route_id: str, departure_date: str) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"Iniciando proceso para ruta {route_id} en fecha {departure_date}..."
            )
...
def main():
    ...
    parser.add_argument(
        "--route", type=str, required=True, help="ID de la ruta (ej. MAD-BAR)"
    )
    parser.add_argument(
        "--date", type=str, required=True, help="Fecha de salida (YYYY-MM-DD)"
    )
